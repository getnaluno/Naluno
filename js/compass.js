...
function resetComposer(){
  ...
  composerType = 'video'; composerItems = []; activeComposerItemIndex = -1; composerTransition = 'fade';
  document.querySelectorAll('.type-chip').forEach(c=>c.classList.toggle('active', c.dataset.type==='video'));
  $('mediaFileInput').accept = 'video/*';
  $('mediaFileInput').value = '';
  $('uploadDropLabel').textContent = 'Choose videos from your library';
...
$('uploadDrop').onclick = function(){
  // Fresh <input> each pick — reused inputs + extension lists make Google Photos "Preparing".
  const inp = document.createElement('input');
  inp.type = 'file';
  inp.accept = composerType === 'video' ? 'video/*' : (composerType === 'photo' ? 'image/*' : 'video/*,image/*');
  inp.multiple = true;
    ...
    for(const seg of newSegments){
      ...
      if(seg.type==='video'){
        ...
        try{
          ...
          let uploadSrc = source;
          try{
            const f = (source instanceof Blob) ? source : null;
            if(f && typeof nalunoSniffIsHevc === 'function' && await nalunoSniffIsHevc(f) && typeof nalunoTranscodeToWeb === 'function'){
              const dur = seg.duration || 0;
              if(dur > 0 && dur <= 360){
                if(hasVideo) setBgProgress((videoIndex-1)/totalVideos, 'Converting Samsung video…');
                uploadSrc = await nalunoTranscodeToWeb(f, function(p){
                  if(hasVideo) setBgProgress(((videoIndex-1)+p)/totalVideos, 'Converting… ' + Math.round((p||0)*100) + '%');
                }, Math.min(360, dur + 1));
              }
            }
          }catch(convErr){ console.warn('[signal] HEVC convert skipped', convErr); }
          const videoUrl = await uploadVideoToR2(uploadSrc);
