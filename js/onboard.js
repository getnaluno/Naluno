...
function nalunoPrivacyHtml(){
  return ''
    ...
    + '<p>Naluno does not ask for a phone number or a SIM card. It does not sell your personal information. Contribution points inside the app are not money and are not a payment. Creator Support takes you to pay only when that step is available. If it is not, nothing is charged.</p>'
    ...
    + '<p>You can edit Callsign, remove a photo, turn Find Naluno off, hide the weather strip, lock Compass, refuse notifications, and sign out. You can close the Callsign from Callsign. Compass and getnaluno.com can also reach the people who run Naluno. Some copies (for example a Signal already expired, or a Band already wiped) are already gone. A closed Callsign can be restored.</p>'
    + '<h3>14. Children</h3>'
...
function nalunoTermsHtml(){
  return ''
    ...
    + '<li>Pretend contribution points are money, or pretend a payment went through when it did not. Contribution points are not currency.</li>'
    ...
    + '<p>You may sign out at any time. You may close the Callsign from Callsign. We may suspend or close an account that breaks these terms, harms others, or puts the service at risk. A closed Callsign can be restored. Band wipes, Signal expiry, and encryption still apply — we cannot hand you a Band that has already been deleted.</p>'
