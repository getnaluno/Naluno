...
if('serviceWorker' in navigator){
  navigator.serviceWorker.register('/sw.js?v=20260925b', { scope: '/', updateViaCache: 'none' })
  ...
  try{
    ...
    navigator.serviceWorker.addEventListener('controllerchange', function(){
      if(reloaded) return;
      try{ if(sessionStorage.getItem('nalunoSwReload') === '20260925b') return; }catch(_){}
      reloaded = true;
      try{ sessionStorage.setItem('nalunoSwReload', '20260925b'); }catch(_){}
      location.reload();
    });
  }catch(_){}
...
if(typeof firebase !== 'undefined' && firebase.messaging){
  try{
    ...
    fgMessaging.onMessage(payload=>{
      ...
      if(typeof nalunoSaveReceipt === 'function'){
        nalunoSaveReceipt({
          pingId: data.pingId || '',
          id: data.pingId || data.callId || data.broadcastId || '',
          type: data.type || '',
          arrivedAt: Date.now(),
        });
      }
